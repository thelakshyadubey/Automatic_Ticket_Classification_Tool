...
st.set_page_config(page_title="Create ML Model", layout="wide")
...
# After all tabs (or before inject_custom_css)
with st.expander("🔁 Load a previously trained model"):
    st.header('Load your saved model')
    uploaded_model = st.file_uploader("Upload your .pk1 model file", type=["pk1", "pkl"])

    if uploaded_model is not None:
        with st.spinner('Loading model...'):
            st.session_state['svm_classifier'] = joblib.load(uploaded_model)
